...
Code must adhere to the style rules set out and enforced by the linting tooling
and automated linting systems which check all code submitted to the repository,
both on push and on pull requests being opened.

You should run `pnpm lint` prior to submitting pull requests (unless you are
100% confident you are already following them, of course, e.g. via IDE support).

Any pull requests which do not pass the automated linting systems will not be
merged until the are passing.
